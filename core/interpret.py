...
def interpret_locally(result: dict, inputs: dict, stress: dict | None = None) -> str:
    ...
    if flow < 0:  # modo retiro
        ...
        lines.append(
            ...
            f"de los {years} años en **{ruin*100:.1f}%** de los 10.000 escenarios — un riesgo "
        ...
        lines.append(
            f"Entre el escenario **pesimista** (P5 {_money(p5)}) y el **optimista** "
            f"(P95 {_money(p95)}) hay ~{spread:.1f}× de diferencia. Esa amplitud es esperable: "
            f"la incertidumbre a 20 años es enorme y conviene planificar con el rango, no con un número.")

    if inputs.get("target"):
        prob = result.get("prob_target") or 0.0
        verdict = "alta" if prob >= 0.70 else "moderada" if prob >= 0.50 else "baja"
        lines.append(
            f"Tu meta de {_money(inputs['target'])} se alcanzó en **{prob*100:.1f}%** de los "
    ...
    lines.append(
        f"Habrá baches en el camino: el **drawdown típico** (caída pico-a-valle) fue "
        f"~{dd*100:.0f}%. Soportarlo sin vender en pánico es parte del plan a largo plazo.")

    if inputs.get("distribution") == "t-student":
        lines.append("Usaste el modelo de **colas gordas** (t-Student): suma realismo a las "
                     "caídas bruscas de corto/mediano plazo.")

    fees, tax = inputs.get("annual_fees_pct", 0.0), inputs.get("annual_tax_on_gains_pct", 0.0)
    if fees or tax:
        lines.append(
            f"Incluiste costos: **{fees:.1f}% de fees** anuales y **{tax:.0f}% de impuesto** sobre "
            f"ganancias. Por el interés compuesto, erosionan el resultado más de lo que parece.")

    if stress and stress.get("events"):
        worst = max(stress["events"], key=lambda e: e["portfolio_drawdown"])
        lines.append(
            f"**Stress test**: un evento como *{worst['name']}* implicaría para tu portafolio una "
            f"caída estimada de ~{worst['portfolio_drawdown']*100:.0f}% "
            f"(beta {stress['beta']:.2f} al S&P 500). Es una estimación de magnitud, no una predicción.")
